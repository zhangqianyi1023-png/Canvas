...
export const getVideoEditorResolutionPreset = (resolution) => (
  VIDEO_EDITOR_RESOLUTION_PRESETS.find(item => item.value === resolution)
...
export const getClipMaxDuration = (clip, inPoint = clip?.inPoint) => {
  ...
  if (clip?.type !== 'video' || sourceDuration <= 0) return 300;
  const safeInPoint = normalizeTimelineNumber(inPoint, 0, 0, sourceDuration);
  return Math.max(MIN_VIDEO_EDITOR_CLIP_DURATION, sourceDuration - safeInPoint);
};
...
export const normalizeVideoEditorTimeline = (timeline) => {
  ...
  return {
    ...
    clips: clips.map((clip, index) => {
      const type = clip.type === 'video' ? 'video' : 'image';
      ...
      const maxDuration = getClipMaxDuration({ ...clip, type, sourceDuration }, inPoint);
      return {
        ...
        duration: normalizeTimelineNumber(clip.duration, type === 'video' ? (sourceDuration || 4) : 3, MIN_VIDEO_EDITOR_CLIP_DURATION, maxDuration),
        inPoint,
...
export const getTimelinePlaybackState = (clips = [], time = 0) => {
  ...
  return {
    clip,
    clipOffset,
    mediaTime: normalizeTimelineNumber(clip.inPoint, 0, 0) + clipOffset,
  };
};

export const createClipFromSource = (source, start = 0) => {
  ...
  return {
    ...
    type,
    name: source.name || (type === 'video' ? '视频素材' : '图片素材'),
    start,
    duration: type === 'video' ? (sourceDuration || 4) : 3,
    inPoint: 0,
    sourceDuration: sourceDuration || undefined,
    aspectRatio: source.aspectRatio ? normalizeAspectRatioNumber(source.aspectRatio) : undefined,
    transform: {
      x: 50,
      y: 50,
      scale: 1,
...
export const splitClipAtTime = (clips = [], clipId, time) => {
  ...
  const rightClip = {
    ...
    inPoint: normalizeTimelineNumber(clip.inPoint, 0, 0) + leftDuration,
    name: `${clip.name} 片段`,
  };
    ...
    clips.map((clip) => {
      ...
      return {
        ...
        duration: normalizeTimelineNumber(
          ...
          clip.duration,
          MIN_VIDEO_EDITOR_CLIP_DURATION,
          getClipMaxDuration(clip, inPoint),
        ),
        inPoint,
      };
    }),
